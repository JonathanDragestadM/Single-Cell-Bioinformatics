...
snRNAseq.integrated <- IntegrateData(anchorset = anchors,
                                     dims = 1:16)
```

Try version from https://github.com/satijalab/seurat/issues/1889
...
rna.names <- rownames(snRNAseq.list[[1]][["RNA"]])

anchors <- FindIntegrationAnchors(
  object.list = snRNAseq.list, 
  assay = rep("RNA",length(snRNAseq.list)), 
  anchor.features=rna.names, 
  dims=1:16)
...
```{r}
#| eval: false
saveRDS(anchors, file = "../data/04_anchors")
```
