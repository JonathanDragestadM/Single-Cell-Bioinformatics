...
data <- readRDS("/home/people/puqu/projects/Single-Cell-Bioinformatics/data/05_RNA_seurat_singleR_subset_ann")
```
...
data_crc <- subset(data, subset = condition == 'CRC')
data_norm <- subset(data, subset = condition == 'norm')

Idents(data)

#Create a CellChat object
cellchat_crc <- createCellChat(object = data_crc, meta = data_crc@meta.data, group.by = "cell_type_singleR")
cellchat_norm <- createCellChat(object = data_norm, meta = data_norm@meta.data, group.by = "cell_type_singleR")
...
cellchat_crc <- subsetData(cellchat_crc) # This step is necessary even if using the whole database
